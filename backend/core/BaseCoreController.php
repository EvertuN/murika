...
class BaseCoreController {
    ...
    protected $createRequireAdmin;
    protected $updateRequireAdmin;
    protected $deleteRequireAdmin;
    protected $logField;
    protected $softDelete;
    protected $uniqueChecks;
    protected $deleteBlockActiveChildren;
    ...
    public function __construct($db, array $config) {
        ...
        $this->createRequireAdmin = $config['createRequireAdmin'] ?? $this->requireAdmin;
        $this->updateRequireAdmin = $config['updateRequireAdmin'] ?? $this->requireAdmin;
        $this->deleteRequireAdmin = $config['deleteRequireAdmin'] ?? $this->requireAdmin;
        $this->softDelete = $config['softDelete'] ?? false;
        $this->uniqueChecks = $config['uniqueChecks'] ?? [];
        $this->deleteBlockActiveChildren = $config['deleteBlockActiveChildren'] ?? [];
    ...
    public function handleRequest() {
        ...
        try {
            ...
            return apiExceptionResponse($e, 'BaseCoreController::handleRequest');
        }
    }
    ...
    protected function create() {
        ...
        if ($this->requireAuth) {
            ...
            if ($this->createRequireAdmin && !isAdmin()) {
                return $this->jsonResponse(false, 'Acesso negado. Apenas administradores podem cadastrar.');
            }
        }
        ...
        try {
            ...
            return apiExceptionResponse($e, 'BaseCoreController::create', 'Não foi possível cadastrar o registro.');
        }
    }
    ...
    protected function read() {
        try {
            ...
            if ($this->softDelete) {
                $selectFields[] = "{$this->table}.is_deleted";
                $selectFields[] = "{$this->table}.deleted_at";
                $selectFields[] = "{$this->table}.deleted_by";
            }
            ...
            $whereParts = [];
            if ($this->softDelete && (!$this->requireAuth || !function_exists('isAdmin') || !isAdmin())) {
                $whereParts[] = "{$this->table}.is_deleted = 0";
            }
            ...
            return apiExceptionResponse($e, 'BaseCoreController::read', 'Não foi possível carregar os registros.');
        }
    }
    ...
    protected function update() {
        ...
        if ($this->requireAuth) {
            ...
            if ($this->updateRequireAdmin && !isAdmin()) {
        ...
        try {
            ...
            return apiExceptionResponse($e, 'BaseCoreController::update', 'Não foi possível atualizar o registro.');
        }
    }
    ...
    protected function delete() {
        ...
        if ($this->requireAuth) {
            ...
            if ($this->deleteRequireAdmin && !isAdmin()) {
        ...
        if (!$this->beforeDelete($id)) {
            if (isset($this->tempLogData['delete_error'])) {
                return $this->jsonResponse(false, $this->tempLogData['delete_error']);
            }
        ...
        try {
            if ($this->softDelete) {
                ...
                $stmt = $this->db->prepare($sql);
                $stmt->bindValue(':deleted_by', $userId ?: null);
                $stmt->bindValue(':id', $id, PDO::PARAM_INT);
                $stmt->execute();
            } else {
            ...
            return apiExceptionResponse($e, 'BaseCoreController::delete', 'Não foi possível excluir o registro.');
        }
    }

    /**
     * ACTIVATE - Reativa registro inativado por soft delete
     */
    protected function activate() {
        if (!$this->softDelete) {
            return $this->jsonResponse(false, 'Este registro não suporta ativação.');
        }
        ...
        if ($this->requireAuth) {
            ...
            if (!isAdmin()) {
                return $this->jsonResponse(false, 'Acesso negado. Apenas administradores podem ativar.');
            }
        }
        ...
        try {
            ...
            $this->oldData = $stmtOld->fetch(PDO::FETCH_ASSOC) ?: [];
            if (!$this->oldData) {
                return $this->jsonResponse(false, 'Registro não encontrado.');
            }

            $stmt = $this->db->prepare("UPDATE {$this->table} SET is_deleted = 0, deleted_at = NULL, deleted_by = NULL WHERE {$this->primaryKey} = :id");
            $stmt->bindValue(':id', $id, PDO::PARAM_INT);
            $stmt->execute();

            $name = $this->logField && isset($this->oldData[$this->logField]) ? $this->oldData[$this->logField] : '';
            Logger::logCrud($this->db, $this->table, 'ACTIVATE', $id, $name);

            return $this->jsonResponse(true, 'Ativado com sucesso!');
        } catch (PDOException $e) {
            return apiExceptionResponse($e, 'BaseCoreController::activate', 'Não foi possível ativar o registro.');
        }
    }
     ...
     */
    protected function jsonResponse($success, $message, $data = [], $status = null) {
        ...
        if (!$success && !isset($response['code'])) {
    ...
    protected function beforeDelete($id) {
        foreach ($this->deleteBlockActiveChildren as $rule) {
            try {
                $childTable = $rule['table'];
                $childField = $rule['field'];
                $message = $rule['message'] ?? 'Não é possível deletar este registro.';
                $activeColumn = $rule['activeColumn'] ?? 'is_deleted';
                $stmt = $this->db->prepare("SELECT COUNT(*) FROM {$childTable} WHERE {$childField} = :id AND {$activeColumn} = 0");
                $stmt->execute([':id' => $id]);
                if (intval($stmt->fetchColumn()) > 0) {
                    $this->tempLogData['delete_error'] = $message;
                    return false;
                }
            } catch (Exception $e) {
                return false;
    ...
    protected function customValidation($data) {
        foreach ($this->uniqueChecks as $check) {
            ...
            $expression = $check['expression'] ?? $field;
            $sql = "SELECT {$this->primaryKey}, is_deleted FROM {$this->table} WHERE {$expression} = :value";
            if ($excludeId > 0) {
            ...
            if ($existing) {
                $inactiveMessage = $check['inactiveMessage'] ?? 'Cadastro já existe inativado. Contate um administrador para ativar.';
                $activeMessage = $check['activeMessage'] ?? 'Cadastro já existente.';
                return [
                    'valid' => false,
                    'message' => intval($existing['is_deleted'] ?? 0) === 1 ? $inactiveMessage : $activeMessage
                ];
            }
        }
